...
function Torah(props) {
  ...
  for (let i = startChapter - 1; i < endChapter; i++) {
    ...
    heb.push(<b key={`torah-text-hebrew-perek-${sefer}-${parasha.id}-${i}`}>פרק {gematriya(i + 1)}</b>);
    eng.push(<b key={`torah-text-english-perek-${sefer}-${parasha.id}-${i}`}>Perek {i + 1}</b>);
    par.push((
      <div key={`torah-text-parallel-perek-${sefer}-${parasha.id}-${i}`}>
    ...
    for (let j = startPassuk - 1; j < endPassuk; j++) {
      heb.push(<p key={`torah-text-hebrew-passuk-${sefer}-${parasha.id}-${i}-${j}`}><b>{gematriya(j + 1)}. </b>{hebrewChapter[j]}</p>);
      eng.push(<p key={`torah-text-english-passuk-${sefer}-${parasha.id}-${i}-${j}`}><b>{j + 1}. </b>{englishChapter[j]}</p>);
      par.push((
        <div key={`torah-text-parallel-passuk-${sefer}-${parasha.id}-${i}-${j}`}>
          <div className='row valign-wrapper hide-on-small-only'>
