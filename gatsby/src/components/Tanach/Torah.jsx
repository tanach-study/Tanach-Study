...
function Torah(props) {
  ...
  for (let i = startChapter - 1; i < endChapter; i++) {
    ...
    heb.push((
      <b key={`torah-text-hebrew-perek-${sefer}-${parasha}-${i}`}>
        פרק {gematriya(i + 1)}
      </b>
    ));
    eng.push((
      <b key={`torah-text-english-perek-${sefer}-${parasha}-${i}`}>
        Perek {i + 1}
      </b>
    ));
    par.push((
    ...
    const endPassuk = (i === endChapter - 1 && endVerse < hebrewChapter.length)
      ? endVerse
      : hebrewChapter.length;
    for (let j = startPassuk - 1; j < endPassuk; j++) {
      heb.push((
        <p key={`torah-text-hebrew-passuk-${sefer}-${parasha}-${i}-${j}`}>
          <b>{gematriya(j + 1)}. </b>
          {hebrewChapter[j]}
        </p>
      ));
      eng.push((
        <p key={`torah-text-english-passuk-${sefer}-${parasha}-${i}-${j}`}>
